...
public class DatabaseHelper extends SQLiteOpenHelper
{
    ...
    public long insertFood(Food food)
    {
        ...
        contentValues.put(Util.FOOD_LATITUDE, food.getLatitude());
        contentValues.put(Util.FOOD_LONGITUDE, food.getLongitude());
    ...
    public List<Food> fetchAllFoods()
    {
        ...
        if (cursor.moveToFirst())
        {
            do {
                ...
                food.setTime(cursor.getString(cursor.getColumnIndex(Util.FOOD_PICKUP_TIMES)));
                ...
                food.setLatitude(cursor.getDouble(cursor.getColumnIndex(Util.FOOD_LATITUDE)));
                food.setLongitude(cursor.getDouble(cursor.getColumnIndex(Util.FOOD_LONGITUDE)));
    ...
    // Method to fetch specific food details
    public Food fetchFoodInfo(int foodId)
    {
        SQLiteDatabase db = this.getReadableDatabase();

        String selectFoodInfo = "SELECT * FROM " + Util.FOOD_TABLE_NAME + " WHERE " + Util.FOOD_ID + "= ?";
        String[] param = new String[] {String.valueOf(foodId)};

        Cursor cursor = db.rawQuery(selectFoodInfo, param);

        // Variables
        byte[] imageBlob;
        String title, description, time, location;
        int quantity;
        long date;
        ...
        if (cursor.moveToFirst())
        {
            do {
                ...
                imageBlob = cursor.getBlob(cursor.getColumnIndex(Util.FOOD_IMAGE));
                title = cursor.getString(cursor.getColumnIndex(Util.FOOD_TITLE));
                description = cursor.getString(cursor.getColumnIndex(Util.FOOD_DESCRIPTION));
                date = cursor.getLong(cursor.getColumnIndex(Util.FOOD_DATE));
                time = cursor.getString(cursor.getColumnIndex(Util.FOOD_PICKUP_TIMES));
                quantity = cursor.getInt(cursor.getColumnIndex(Util.FOOD_QUANTITY));
                location = cursor.getString(cursor.getColumnIndex(Util.FOOD_LOCATION));
                latitude = cursor.getDouble(cursor.getColumnIndex(Util.FOOD_LATITUDE));
                longitude = cursor.getDouble(cursor.getColumnIndex(Util.FOOD_LONGITUDE));
        ...
        return new Food(imageBitmap, title, description, date, time, quantity, location, latitude, longitude);
    }


    // Method to fetch food object from list of food Id
    public List<Food> fetchAllCartFood(ArrayList<Integer> cartList)
    {
        List<Food> cartFoodList = new ArrayList<>();

        for (int i = 0; i < cartList.size(); i++)
        {
            cartFoodList.add(fetchFoodInfo(cartList.get(i)));
        }
